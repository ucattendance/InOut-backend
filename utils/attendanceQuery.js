...
const userIdFilters = (userId) => {
  ...
  // Normal attendance rows where user is MongoDB ObjectId
  ...
  // Legacy attendance rows where user stores employeeId
  // such as UC0075 instead of MongoDB ObjectId.
  filters.push({
    $expr: {
      $eq: [{ $toString: '$user' }, id],
    },
...
const loadEmployeeMaps = async () => {
  ...
  if (
    employeeMapsCache &&
    now - employeeMapsCacheTime < EMPLOYEE_MAPS_TTL
  ) {
...
/**
 * Resolve employee from attendance.user
 * (ObjectId, id string, or employeeId like UC0031).
 */
const resolveEmployee = (userRef, maps) => {
  ...
  const direct =
    maps.byId.get(raw) ||
    maps.byEmpId.get(raw);

  if (direct) return direct;

  if (mongoose.Types.ObjectId.isValid(raw)) {
    const byOid = maps.byId.get(
      String(new mongoose.Types.ObjectId(raw))
    );
  ...
  for (const [empId, user] of maps.byEmpId) {
    if (String(empId).toLowerCase() === lower) {
      return user;
    }
  }
...
/**
 * Fetch attendance for a calendar day,
 * including legacy rows that only have ObjectId time.
 */
const fetchAttendanceInRange = async (start, end) => {
  ...
  return [...byId.values()].sort((a, b) => {
    const ta =
      getAttendanceTimestamp(a)?.getTime() || 0;

    const tb =
      getAttendanceTimestamp(b)?.getTime() || 0;
